...
{-| Whether or not a scale domain should be independent of others in a
...
{-| Specify a list of boolean data values.
-}
...
{-| Specify a date-time data value.
-}
...
{-| Specify a day of the month as an integer.
-}
dtDate : Int -> DateTime
dtDate =
    DTDate
...
{-| Specify an hour of the day (0=midnight, 1=1am, 23=11pm etc.) an integer.
-}
dtHour : Int -> DateTime
dtHour =
    DTHours
...
{-| Specify a millisecond of a second (0-999).
-}
...
{-| Specify a minute of an hour (0-59).
-}
...
{-| Specify a month as an integer (1=January, 2=February etc.).
-}
...
{-| Specify a year quarter as an integer.
-}
...
{-| Specify a list of date-time data values.
-}
...
{-| Specify a second of a minute (0-59).
-}
...
a set of small multiples. Used where the encoding of the visualization in small
...
{-| Specify the 1d window over which data imputation values are generated. The two
...
{-| Specify the key values to be considered for imputation.
-}
...
{-| Specify the key values to be considered for imputation as a sequence of numbers
between a start (first parameter), to less than an end (second value) in steps of
the third parameter.
-}
...
{-| Specify the imputation method to use when replacing values.
-}
...
{-| Specify the new value to use when imputing with the method `ImValue`.
-}
...
The following returns the values in the `age` and `height` fields from
...
{-| Specify the the number of pixels before which the first line dash is drawn.
-}
...
{-| Specify the source of a mark's tooltip content.
-}
...
{-| Set the iputation rules for a mark channel. See the
[Vega-Lite impute documentation](https://vega.github.io/vega-lite/docs/impute.html)
-}
...
{-| Specify a list of numeric data values.
-}
...
{-| Specify a named aggregation operation on a field. The first parameter is the
aggregation operation to use; the second the name of the field in which to apply
it and the third the name to be given to this transformation.
...
{-| Set the iputation rules for a position channel. See the
[Vega-Lite impute documentation](https://vega.github.io/vega-lite/docs/impute.html)
-}
...
small multiples. Used when the encoding of the visualization in small multiples
is largely identical, but the data field used in each might vary. When a list of
fields is identified with `repeat` you also need to define a full specification
...
views share channel encodings. This allows, for example, two different color encodings
to be created in a layered view, which would otherwise share color channels between
layers. Each resolution rule should be in a tuple pairing the channel to which it
applies and the rule type. The first parameter identifies the type of resolution.
...
the name of data the field and its measurement type.
-}
...
{-| Specify the stack offset when applying a stack transformation.
-}
...
{-| Specify the ordering within a stack when applying a stack transformation.
-}
...
{-| Specify a list of string data values.
-}
...
{-| Specify an data field for which to compute an operation. Not needed for
...
ignore peer values (those considered identical by the sort criteria).
-}
...
{-| Specify a window transform for performing calculations over sorted groups of
...
{-| Specify a comparator for sorting data objects within a window transform.
-}
