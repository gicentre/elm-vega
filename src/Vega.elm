...
[axMaxExtent](#axMaxExtent), [axMinExtent](#axMinExtent), [axOffset](#axOffset),
[axPosition](#axPosition), [axTickCount](#axTickCount), [axTicks](#axTicks),
[axTickSize](#axTickSize), [axTitle](#axTitle), [axTitlePadding](#axTitlePadding),
...
Generated by [feSignal](#feSignal) and [feName](#feName).
-}
type Feature
    = FeSignal String
    | FeName String
...
{-| Create a data reference from a list literals. Useful when combining with data
references from existing data streams. For example
...
{-| Specify the name of the a new signal to to capture the output of generated by
...
{-| Perform a flattening transform that maps array-valued fields to a set of
individual data objects, one per array entry. This version generates the output
fields with names corresponding to the array field used. To customise the names
of these fields use [trFlattenAs](#trFlattenAs).
For details see the
...
{-| Perform a flattening transform that maps array-valued fields to a set of
individual data objects, one per array entry. This version names the output fields
...
{-| Perform an imputute transform which generates new values for missing data.
...
in a similary way to [trAggregate](#trAggregate) but which is then joined back to
