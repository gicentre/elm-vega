...
[kdMaxSteps](#kdMaxSteps), [kdSteps](#kdSteps) and [kdAs](#kdAs).
-}
type KdeProperty
...
{-| The minimum (first parameter) and maximum (second parameter) from which to sample
the distribution for a KDE transform. If not specified, the full domain extent is used.
-}
kdExtent : Num -> Num -> KdeProperty
kdExtent =
    KdExtent
...
{-| Name of loess (locally estimated regression) output fields representing the
...
kdeProperty kp =
    case kp of
        ...
        KdExtent nMin nMax ->
            ( "extent", JE.list numSpec [ nMin, nMax ] )
