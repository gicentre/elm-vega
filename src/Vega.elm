...
The parameters are the operation to apply, the input field (or `strNull` if no input
...
The example below calculates the average over an unbounded window:
    ...
    trWindow [ wnAggOperation Mean (str "IMDB_Rating") "avScore" ]
...
-}
wnAggOperation : Operation -> Field -> String -> WindowOperation
...
wnOperation op outField =
    WnOperation op Nothing strNull outField
...
the field to apply it to and its output field name.
    ...
    trWindow [ wnOperationOn Lag (Just 5) (str "temperature" "oldTemp" ] []

For details see the
[Vega window transform documentation](https://vega.github.io/vega/docs/transforms/window/)

-}
wnOperationOn : WOperation -> Maybe Num -> Field -> String -> WindowOperation
wnOperationOn =
    WnOperation
