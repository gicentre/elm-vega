...
by [wnOperation](#wnOperation) and [wnAggOperation](#wnAggOperation).
For details see the
[Vega window transform documentation](https://vega.github.io/vega/docs/transforms/window/)
-}
type WindowOperation
    = WnOperation WOperation (Maybe Num) Field String
    | WnAggOperation Operation (Maybe Num) Field String
...
The example below calcualtes the average over an unbounded window:

    TODO: Check this works as expected.

    trWindow [ wnAggOperation Mean Nothing (str "IMDB_Rating") "avScore" ]
...
-}
wnAggOperation : Operation -> Maybe Num -> Field -> String -> WindowOperation
wnAggOperation =
    WnAggOperation
...
In the example below the operation `RowNumber` does not require a parameter so
`Nothing` is provided and because `RowNumber` does not operate on a specific field,
`strNull` is provided as the field name.
...
-}
wnOperation : WOperation -> Maybe Num -> Field -> String -> WindowOperation
wnOperation =
    WnOperation
...
Comparators can be created with [coField](#coField) and [coOrder](#coOrder).

    TODO: XXX Provide example

If two objects are considered equal by the comparator they are considered 'peers'.
If no sorting comparator is specified, data objects are processed in the order
they are observed with no peers. For details see the
[Vega window transform documentation](https://vega.github.io/vega/docs/transforms/window/)

-}
wnSort : List Comparator -> WindowProperty
wnSort =
    WnSort
...
is redundancy in the list of groupBy fields (as there is when binning for example).
    ...
    transform
        [ trBin (str "examScore") (nums [ 0, 100 ]) []
        , trAggregate
            [ agKey (str "bin0")
...
{-| Create the axes used to visualize spatial scale mappings.
    ...
    ax =
        axes
            ...
            << axis "myYScale" SLeft [ axTickCount 5, axZIndex 1 ]
...
-}
axTitlePadding : Num -> AxisProperty
axTitlePadding =
    AxTitlePadding
...
[Vega comparison documentation](https://vega.github.io/vega/docs/types/#Compare)
-}
...
[Vega data documentation](https://vega.github.io/vega/docs/data).
      ...
      dataSource
          ...
          , data "ageGroups" [ daSource "pop" ] |> transform [ trAggregate [ agGroupBy [ str "age" ] ] ]
          ]
...
to mark item properties, append the prefix `datum` to a field name.
...
the comparator are tree node objects, not input data objects.
...
{-| Specify a comparator for sorting sibling nodes as part of a tree layout transform.
For details see the
[Vega tree transform documentation](https://vega.github.io/vega/docs/transforms/tree/).
-}
teSort : Comparator -> TreeProperty
teSort =
    TeSort
...
{-| Specify the comparator for sorting sibling nodes during a treemap layout transform.
For details see the
[Vega treemap transform documentation](https://vega.github.io/vega/docs/transforms/treemap/).
-}
tmSort : Comparator -> TreemapProperty
tmSort =
    TmSort
...
treemapProperty tp =
    case tp of
        ...
        TmSort c ->
            ( "sort", JE.object [ comparatorProperty c ] )
...
windowProperty wp =
    case wp of
        WnSort cs ->
            ( "sort", JE.object (List.map comparatorProperty cs) )
